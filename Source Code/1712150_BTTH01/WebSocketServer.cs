...
namespace _1712150_BTTH01
{
    public static class WebSocketServer
    {
        ...
        public static void Start(string uriPrefix)
        {
            ...
            {
                Console.WriteLine("Connect browser for a basic login web page.");
                    ...
                    if (ServerIsRunning)
                    {
                        ...
                        else
                        {
                            if (context.Request.AcceptTypes.Contains("text/html"))
                            {
                                ...
                                ReadOnlyMemory<byte> HtmlPage = new ReadOnlyMemory<byte>(Encoding.UTF8.GetBytes(HtmlClient.HTML));
                ...
                while (socket.State != WebSocketState.Closed && socket.State != WebSocketState.Aborted && !loopToken.IsCancellationRequested)
                {
                    ...
                    if (!loopToken.IsCancellationRequested)
                    {
                        ...
                        if (client.Socket.State == WebSocketState.Open)
                        {
                            ...
                            Console.WriteLine($"Socket {client.SocketId}: Sending data to client.");
